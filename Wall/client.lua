...
RegisterCommand("wall",function()
    ...
    if wall then
		drawNotification("~g~Wall ligado com sucesso! [MIAMI-AC].")
		arc.reportwallLog("WALL ON")
		print("^2WALL ON!")
		PlaySoundFrontend(-1, "CONFIRM_BEEP", "HUD_MINI_GAME_SOUNDSET", 0)
	else
		drawNotification("~r~~g~Wall desligado com sucesso! [MIAMI-AC].")
						...
						if admin and (PlayerPedId() ~= GetPlayerPed(id)) then
							if GetPlayerPed(id) ~= -1 and players[id] ~= nil then
								...
								if vidaPorcento == 0 then
									vidaPorcento = "~h~~r~Identificado como morto~w~"
								end
								...
								if distance <= Config.Distanciaesp then
									AliggShop = 1
									DrawText3D(x2, y2, z2+1.7,"~b~Distância: "..distance.." m~w~\nID: ~b~"..players[id].."\nNome:~w~  "..name.."\n~g~VIDA ATUAL: ~b~"..vidaPorcento.."~w~")
									DrawText3D(x2, y2, z2-1.5,"~g~"..arma.."~w~")
								end
							end
						end
					end
					...
					if ((NetworkIsPlayerActive( id )) and GetPlayerPed(id) ~= PlayerPedId()) then	
						if admin and (PlayerPedId() ~= GetPlayerPed(id)) then
							if GetPlayerPed(id) ~= -1 and players[id] ~= nil then
								...
								if distance <= Config.Distanciaesp then
									AliggShop = 1
									DrawLine(gete, entityc, cordes, x, y, z, 0, 0, 0, 255)
								end
